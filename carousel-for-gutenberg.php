<?php
/**
 * Plugin Name: Carousel for Gutenberg
 * Plugin URI:
 * Description: Transforme n'importe quel bloc Gutenberg en carousel performant avec la classe .cfg-carousel. 100% CSS, zéro JavaScript, compatible avec tous les thèmes.
 * Version: 1.0.1
 * Author: weblazer35
 * Author URI: https://weblazer.fr
 * License: GPL v2 or later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: carousel-for-gutenberg
 * Domain Path: /languages
 * Requires at least: 6.0
 * Tested up to: 6.8
 * Requires PHP: 7.4
 *
 * @package CarouselForGutenberg
 */
...
define('CAROUSEL_FOR_GUTENBERG_VERSION', '1.0.1');
define('CAROUSEL_FOR_GUTENBERG_PLUGIN_URL', plugin_dir_url(__FILE__));
define('CAROUSEL_FOR_GUTENBERG_PLUGIN_PATH', plugin_dir_path(__FILE__));

/**
 * Classe principale du plugin Carousel for Gutenberg
 */
class CarouselForGutenberg
{
    /**
    ...
    public function enqueue_block_assets(): void
    {
        wp_enqueue_style(
            'carousel-for-gutenberg',
            CAROUSEL_FOR_GUTENBERG_PLUGIN_URL . 'assets/css/carousel.css',
            [],
            CAROUSEL_FOR_GUTENBERG_VERSION
        );
    ...
    private function inject_theme_button_colors(): void
    {
        ...
        if (!empty($button_bg) || !empty($button_color)) {
            wp_add_inline_style('carousel-for-gutenberg', $custom_css);
        }
    }
    ...
    {
        wp_enqueue_script(
            'carousel-for-gutenberg-editor',
            CAROUSEL_FOR_GUTENBERG_PLUGIN_URL . 'assets/js/carousel-button.js',
            [
                'wp-blocks',
                'wp-element',
                'wp-editor',
            ...
            CAROUSEL_FOR_GUTENBERG_VERSION . '-' . time(),
            true
        );
    }
    ...
    {
        // Vérifier si le bloc a la classe 'cfg-carousel'
        $class_name = $block['attrs']['className'] ?? '';
        if (strpos($class_name, 'cfg-carousel') === false) {
        ...
        if (
            ...
            strpos($class_name, 'cfg-carousel-min-width') !== false
        ) {
        ...
        // Chercher la balise avec la classe cfg-carousel
        $pattern = '/(<(?:div|ul|figure)\s+[^>]*class="[^"]*\bcfg-carousel\b[^"]*"[^>]*?)(?:\s+style="([^"]*)")?(\s*>)/i';
...
function carousel_for_gutenberg_activate(): void
{
    ...
    if (version_compare(get_bloginfo('version'), '6.0', '<')) {
        wp_die(
            esc_html__('Ce plugin nécessite WordPress 6.0 ou plus récent.', 'carousel-for-gutenberg'),
            esc_html__('Version WordPress insuffisante', 'carousel-for-gutenberg')
        );
    }
    ...
    if (version_compare(PHP_VERSION, '7.4', '<')) {
        wp_die(
            esc_html__('Ce plugin nécessite PHP 7.4 ou plus récent.', 'carousel-for-gutenberg'),
            esc_html__('Version PHP insuffisante', 'carousel-for-gutenberg')
        );
    }
}
...
CarouselForGutenberg::getInstance();
